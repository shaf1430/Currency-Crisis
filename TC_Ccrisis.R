...
chart<-function (data){
  ...
  pch<- ggplot(data,aes(y=Pct_change1,x=timeline,col=endperiod))+
    ...
    annotate(geom='text',x=1,y=-105,label='1st Yr',hjust=0.5,vjust=0.6,size=11,family='Courier New')+
    annotate(geom='text',x=6,y=-105,label='5th Yr',hjust=0.5,vjust=0.6,size=11,family='Courier New')+
    # y-axis
...
df1 %>%
  ...
  # Add footnotes
  tab_footnote(
    footnote = md("The plots show how much of the U.S. Dollar each currency could buy from 2006 to 2020 end-year."),
